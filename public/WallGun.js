class WallGun {
    ...
    offerPickup(){
        if(currentGun.name != this.gun.name){
            fill(255, 255, 255);
            textSize(30);
            text(this.message + this.cost, clientPlayer.x - this.xTextDisplacement, clientPlayer.y + this.yTextDisplacement);
        }else{
            fill(255, 255, 255);
            textSize(30);
            text(this.message2 + this.ammoCost, clientPlayer.x - 200, clientPlayer.y + 100);
        }
